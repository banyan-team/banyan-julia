...
@testset "Complex usage of BanyanDataFrames on medium dataset" begin
    run_with_job("Filter groupby") do job
        ...
        distances = collect(
            sort(
                combine(
                    groupby(
                        filter(row -> row.trip_distance > 1.0, tripdata),
                        :passenger_count,
                    ),
                    :trip_distance => mean,
                ),
        ...
        @test round.(distances[:, :trip_distance_mean], digits = 3) ==
    ...
    run_with_job("Operations on DateTime") do job
        ...
        setindex!(
            ...
            :start_time,
            map(
                t -> DateTime(t, "yyyy-mm-dd HH:MM:SS"),
        ...
        setindex!(
            ...
            map(
                t -> DateTime(t, "yyyy-mm-dd HH:MM:SS"),
                tripdata[:, :tpep_dropoff_datetime],
            ),
        )
        ...
        tripdata_unique_start_end_of_month = tripdata_unique_start[
            map(
                st -> st > DateTime(2016, 1, 30, 00, 00, 00, 00),
                tripdata_unique_start[:, :start_time],
            ),
            :,
        ]
    ...
    run_with_job("Groups and joins") do job
        ...
        result_1 = rename(result_1, :trip_distance_function => :trip_distance_normalized)
        result_1 = sort(
            combine(groupby(result_1, :passenger_count), :trip_distance_normalized => mean),
        ...
        result_2 = sort(
            combine(groupby(result_2, :passenger_count), :trip_distance_normalized => mean),
            :trip_distance_normalized_mean,
        )
    ...
    run_with_job("Select and subset") do job
        ...
        setindex!(
            ...
            :day,
            map(
                t -> day(DateTime(t, "yyyy-mm-dd HH:MM:SS")),
                tripdata[:, :tpep_pickup_datetime],
            ),
        )
        gdf = groupby(tripdata, :day)
        res = subset(gdf, :trip_distance => d -> d .> mean(d))
