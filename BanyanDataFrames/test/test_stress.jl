...
#         @test sort(names(tripdata_large)) == [
#             "dropoff_datetime",
#             "dropoff_latitude",
#             "dropoff_longitude",
#             "fare_amount",
...
#             "vendor_id",
#         ]
#         @test sort(propertynames(tripdata_large)) == [
#             :dropoff_datetime,
#             :dropoff_latitude,
#             :dropoff_longitude,
#             :fare_amount,
...
#         tripdata_large = dropmissing(tripdata_large)
#         tripdata_large_filtered = filter(row -> (row.passenger_count > 1), tripdata_large)
#         res = transform(
...
#         )
#         res[:, :fraction] = map(
#             (n, s) -> round(s / n, digits = 2),
#             res[:, :n_trips],
#             res[:, :total_trip_distance],
#         )
#         res = sort(res, [:fraction, :n_trips])

#         res_collected = compute(res[:, [:n_trips, :total_trip_distance, :fraction]])

#         @test nrow(tripdata_large_filtered) == 6242979 * n_repeats
#         @test nrow(unique(tripdata_large_filtered)) == 6242979
#         @test length(gdf) == 6
#         @test compute(groupcols(gdf)) == [:passenger_count]
#         @test Set(compute(valuecols(gdf))) == Set([
#             :vendor_id,
#             :pickup_datetime,
#             :dropoff_datetime,
#             :trip_distance,
...
#             :tolls_amount,
#             :total_amount,
#             :is_distance_longer_than_mean,
#         ])
#         @test res_collected[:, :n_trips] ==
#               [15368, 1116069, 4510158, 601379, 2, 3] * n_repeats
#         @test res_collected[:, :total_trip_distance] ==
#               [4239, 325916, 1320006, 178700, 1, 2] * n_repeats
