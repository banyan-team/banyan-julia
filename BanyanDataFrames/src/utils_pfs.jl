...
#     # An optimized version of sync_across that syncs data frames across workers
#     println("In reduce_across at start with get_worker_idx(comm)=$(get_worker_idx(comm)), get_worker_idx()=$(MPI.Initialized() ? get_worker_idx() : -1)")
#     io = IOBuffer()
#     Arrow.write(io, df, compress=:zstd)
#     blob_length = MPI.Allreduce(io.size, +, comm)
#     println("In reduce_across after MPI.Allreduce with get_worker_idx(comm)=$(get_worker_idx(comm)), get_worker_idx()=$(MPI.Initialized() ? get_worker_idx() : -1)")
...
#     # @show reducing_op
#     println("In reduce_across before MPI reduction with get_worker_idx(comm)=$(get_worker_idx(comm)), get_worker_idx()=$(MPI.Initialized() ? get_worker_idx() : -1)")
#     if sync_across
#         MPI.Allreduce!(reducable_buf, reducing_op, comm)
#     else
#         MPI.Reduce!(reducable_buf, reducing_op, to_worker_idx-1, comm)
#     end
#     println("In reduce_across after MPI reduction with get_worker_idx(comm)=$(get_worker_idx(comm)), get_worker_idx()=$(MPI.Initialized() ? get_worker_idx() : -1)")
