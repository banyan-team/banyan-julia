@testset "MapReduce-style computation" begin
    run_with_session("Filling") do job
    ...
    run_with_session("Multiple evaluations apart") do job
        x = BanyanArrays.fill(10.0, 2048)
        x = map(e -> e / 10, x)
    ...
    run_with_session("Multiple evaluations together") do job
        x = BanyanArrays.fill(10.0, 2048)
        x = map(e -> e / 10, x)
        res1 = sum(x)
        res2 = minimum(x)
    ...
    run_with_session("Simple computing") do job
        for _ in 1:8
    ...
    run_with_session("Computing") do job
        # NOTE: This also tests simple writing to and reading from local disk
        x = BanyanArrays.fill(10.0, 2048)
        x = map(e -> e / 10, x)
        @show typeof(x)
    ...
    run_with_session("Re-computing") do job
        x = BanyanArrays.fill(10.0, 2048)
        x_sum = reduce(+, x)
        x = map(e -> e / 10, x)
        write_to_disk(x)
    ...
    run_with_session("Map with multiple values") do job
    ...
    run_with_session("Complex dependency graphs") do job
    ...
    run_with_session("Multiple arrays") do job
    ...
    run_with_session("2D arrays") do job
    ...
    # run_with_session("String arrays") do job
