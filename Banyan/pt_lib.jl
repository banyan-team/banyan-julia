...
default_batches_func = function (src, part, parameters, idx, nbatches) end

default_workers_func = function (src, part, parameters, comm) end

default_lt_func =
    function (src, part, parameters, idx, nbatches, comm, lt_params) end
...
SPLIT_IMPL["Div"]["Batches"] =
    function (src, part, split_params, idx, nbatches)
        part[] = fld(src[], nbatches)
    end

SPLIT_IMPL["Div"]["Workers"] = function (src, part, split_params, comm)
    part[] = fld(src, MPI.Comm_size(comm))
end

SPLIT_IMPL["Div"]["None"] =
    function (src, part, split_params, idx, nbatches, comm, lt_params)
        part[] = flt(split_params[1], nbatches)
    end

SPLIT_IMPL["Replicate"] = Dict()

SPLIT_IMPL["Replicate"]["Batches"] =
    function (src, part, split_params, idx, nbatches)
        part[] = src[]
    end

SPLIT_IMPL["Replicate"]["Workers"] = function (src, part, split_params, comm)
    part[] = src[]
end

SPLIT_IMPL["Replicate"]["Client"] =
    function (src, part, split_params, idx, nbatches, comm, lt_params)
...
SPLIT_IMPL["Block"]["Workers"] =
    function (src, part, split_params, idx, nbatches, comm)
        if src[] == nothing
            part[] = nothing
        else
...
SPLIT_IMPL["Stencil"]["Batches"] =
    function (src, part, split_params, idx, nbatches)
        ...
        first_idx = max(
            min(1 + idx * partition_length, size(src[], dim) + 1) -
            left_overlap,
            0,
        )
        last_idx = min(
            min((idx + 1) * partition_length, size(src[], dim)) +
            right_overlap,
            size(src[], dim),
        )
...
SPLIT_IMPL["Bucket"]["Workers"] = function () end

SPLIT_IMPL["Bucket"]["None"] = default_lt_func

SPLIT_IMPL["Bucket"]["CSV"] = function () end

SPLIT_IMPL["Bucket"]["Arrow"] =
    function (src, part, split_params, idx, nbatches, comm, lt_params)
        hash_col = split_params[1]
...
MERGE_IMPL["Replicate"]["Workers"] = function () end

MERGE_IMPL["Replicate"]["Workers"] = function () end

MERGE_IMPL["Replicate"]["Client"] =
    function (src, part, merge_params, idx, nbatches, comm, lt_params)
...
MERGE_IMPL["Block"]["Workers"] = function (src, part, merge_params, comm)
    if src[] == nothing
...
MERGE_IMPL["Block"]["HDF5"] =
    function (src, part, splitting_parameters, idx, nbatches, comm, lt_params)
        # TODO: Implement this
...
MERGE_IMPL["Stencil"]["Workers"] = function (src, part, merge_params, comm)
    # TODO: Implement this
    ...
    if src == nothing
        ...
        part[] = selectdim(
            part[],
            dim,
            (1+left_overlap):(size(part[], dim)-right_overlap),
        )
        # TODO: Allgather or Allgather!
        src[] = Allgather(part[], comm)
    end
...
MERGE_IMPL["Bucket"]["Batches"] = function () end

MERGE_IMPL["Bucket"]["Workers"] = function () end

MERGE_IMPL["Bucket"]["None"] = function () end

MERGE_IMPL["Bucket"]["CSV"] = function () end

MERGE_IMPL["Bucket"]["Parquet"] = function () end
